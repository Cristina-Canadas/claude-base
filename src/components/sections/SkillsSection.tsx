...
function getDocsUrl(installCommand?: string) {
  const match = installCommand?.match(/--skill\s+(\S+)/);
  return match ? `https://aitmpl.com/component/skill/${match[1]}` : undefined;
}

function SkillCard({ skill }: { skill: SkillItem }) {
  const Icon = CATEGORY_ICONS[skill.category] ?? Wand2;
  const docsUrl = getDocsUrl(skill.installCommand);
    ...
    <Card className="flex h-full min-w-0 flex-col">
        ...
        <div className="flex items-center gap-2">
          ...
          {docsUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              aria-label="Ver documentación de la skill"
              title="Ver documentación de la skill"
      ...
      {skill.installCommand && (
        <div className="mt-4 flex-1">
          ...
          <CopyRow value={skill.installCommand} ariaLabel="Copiar comando de instalación" />
        </div>
      )}
    </Card>
  );
  ...
  return (
    <SectionContainer id="skills">
      <SectionHeading
        eyebrow="Biblioteca"
        title="Skills útiles"
        description="Ejemplos de skills organizadas por categoría, listas para adaptar o usar como punto de partida."
      />
      ...
      <div className="mt-6 flex items-start gap-3 rounded-xl border border-ink-800 bg-ink-900/40 px-4 py-3">
        ...
        <p className="text-sm leading-relaxed text-ink-400">
          ...
          pégalo en tu terminal para añadir la skill al proyecto. Las skills vienen del proyecto
          community{" "}
          <a
